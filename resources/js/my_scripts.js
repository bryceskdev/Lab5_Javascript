...
function loadStatsPage() {
	...
	for (var index = 2; index <table.rows.length; index++) {
		if(parseInt(table.rows[index].cells["2"].innerHTML) > parseInt(table.rows[index].cells["3"].innerHTML)){
			win++;
			table.rows[index].cells["4"].innerHTML = "CU Boulder";
		}else{
			loss++;
			table.rows[index].cells["4"].innerHTML = table.rows[index].cells["1"].innerHTML;
		}
